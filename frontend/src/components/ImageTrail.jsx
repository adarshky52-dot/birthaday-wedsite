...
class ImageTrailVariant7 {
  ...
  showNextImage() {
    ...
    if (this.visibleImagesCount >= this.visibleImagesTotal) {
      ...
      if (oldImg && oldImg.DOM && oldImg.DOM.el) {
...
export default function ImageTrail({ items = [], variant = 1, threshold = 80, imageWidth = 220 }) {
  ...
  useEffect(() => {
    ...
    // Monkey-patch render to support destroyed state clean shutdown
    ...
    return () => {
      // Set destroyed flag to stop any active render RAF loops
      instance.destroyed = true;

      // Clean up window resize event listeners to prevent memory leaks
      if (instance.images) {
        instance.images.forEach(img => {
          if (img && img.resize && typeof window !== 'undefined') {
            window.removeEventListener('resize', img.resize);
          }
        });
      }
    };
  }, [variant, items, threshold, imageWidth]);
